...
{
    public class FoundationTypeRepository : BaseRepository<FoundationType>
    {
        ...
        //            Database.AddInParameter(dbCommand, "@FoundationTypeID", DbType.Int16, p.FoundationTypeID);
        ...
        //            Database.AddInParameter(dbCommand, "@FoundationTypeNotes", DbType.String, p.FoundationTypeNotes);
        ...
        public IEnumerable<FoundationType> GetAll()
        {
            List<FoundationType> FoundationTypees = new List<FoundationType>();
            string SP = ProducteevConstant.StoredProcedures.FoundatoinType.GET_ALL_FOUNDATION_TYPE;
            try
            {
                using (DbCommand dbCommand = Database.GetStoredProcCommand(SP))
                {
                    using (IDataReader reader = Database.ExecuteReader(dbCommand))
                    {
                        if (reader != null && !reader.IsClosed)
                        {
                            while (reader.Read())
                            {
                                var FoundationType = Map(reader);
                                FoundationTypees.Add(FoundationType);
                            }
                            reader.Close();
                        }
                    }
            ...
            return (IEnumerable<FoundationType>)FoundationTypees;
        }
