...
{
    public class ProjectPriorityRepository : BaseRepository<ProjectPriority>
    {
        ...
        //            Database.AddInParameter(dbCommand, "@ProjectPriorityID", DbType.Int16, p.ProjectPriorityID);
        ...
        public IEnumerable<ProjectPriority> GetAll()
        {
            List<ProjectPriority> ProjectPriorityes = new List<ProjectPriority>();
            string SP = ProducteevConstant.StoredProcedures.ProjectPriority.GET_ALL_PROJECT_PRIORITY;
            try
            {
                using (DbCommand dbCommand = Database.GetStoredProcCommand(SP))
                {
                    using (IDataReader reader = Database.ExecuteReader(dbCommand))
                    {
                        if (reader != null && !reader.IsClosed)
                        {
                            while (reader.Read())
                            {
                                var ProjectPriority = Map(reader);
                                ProjectPriorityes.Add(ProjectPriority);
                            }
                            reader.Close();
                        }
                    }
            ...
            {
                throw new Exception("Failed in GetAll() project PRIORITIES", ex);
            }
            return (IEnumerable<ProjectPriority>)ProjectPriorityes;
        }
