...
{
    public class ProjectStatusRepository:BaseRepository<ProjectStatus>
    {   
        protected ProjectStatus Map(IDataReader reader)
        {
        ...
        public IEnumerable<ProjectStatus> GetAll()
        {
            ...
            string SP = ProducteevConstant.StoredProcedures.ProjectStatus.GET_ALL_PROJECT_STATUS;
            try
            {
                using (DbCommand dbCommand = Database.GetStoredProcCommand(SP))
                {
            ...
            return (IEnumerable<ProjectStatus>)projectStatuses;
        }
